...
def wallet_purchase(buyer_id, seller_id, admin_id, product_id,
    ...
    Wallets are locked in ascending user_id order to prevent deadlocks
    when concurrent transactions lock the same wallets in different orders.
    """
    conn = get_connection()
    try:
        # Determine lock order by user_id (ascending) to prevent deadlocks
        participants = [buyer_id, seller_id]
        if admin_id:
            participants.append(admin_id)
        participants.sort()

        # Lock all wallets in sorted order
        wallet_map = {}
        ...
        if admin_id and admin_id in wallet_map:
