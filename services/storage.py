...
def upload_preview_image(file_obj, original_filename: str) -> str:
    ...
    Raises:
        ValueError if the file type is not an allowed image type.
    """
    ext = _ext(original_filename)
    if ext not in ALLOWED_IMAGE_EXTENSIONS:
        raise ValueError(f"Preview must be an image (jpg, jpeg, png, webp). Got '.{ext}'.")

    if is_supabase_enabled():
        url, _ = _upload_supabase(file_obj, original_filename, ext)
    else:
